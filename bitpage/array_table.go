...
func newArrayTable(path string, opts *atOptions, cacheOpts *atCacheOptions) (*arrayTable, error) {
	var err error
	dataFile, err := OpenTable(path, defaultTableOptions)
	if err != nil {
		return nil, err
	}
	...
	headerOffset, err = at.dataFile.alloc(atHeaderSize)
	if err != nil {
		return nil, err
	}
	...
	if opts.useBitrieCompress {
		...
		at.itemsOffset = make([]uint32, 0, 1<<18)
	}
...
func openArrayTable(path string, cacheOpts *atCacheOptions) (*arrayTable, error) {
	...
	tableOpts := &TableOptions{
		OpenType: TableTypeReadMmap,
	}
	dataFile, err := OpenTable(path, tableOpts)
	if err != nil {
		return nil, err
	}
...
func (a *arrayTable) getID() string {
	return a.id
}

func (a *arrayTable) writeHeader() {
	buf := a.dataFile.GetBytes(atHeaderOffset, atHeaderSize)
...
func (a *arrayTable) dataIntBuffer() {
	if a.header.version == atVersionBitrieCompress {
		a.bitrieIndex.SetReader(a.dataFile.GetBytes(0, a.dataSize), atDataOffset)
	} else {
		a.intIndex = a.dataFile.GetBytes(a.header.intIndexOffset, a.getIntIndexSize())
	}

	if a.useMapIndex {
		a.mapIndex.SetReader(a.dataFile.GetBytes(a.header.mapIndexOffset, a.dataSize-a.header.mapIndexOffset))
	}
}
...
func (a *arrayTable) writeSharedInternal(shared int) (uint32, error) {
	...
	itemOffset, err := a.dataFile.alloc(sz)
	if err != nil {
		return 0, err
	}

	a.itemsOffset = append(a.itemsOffset, itemOffset)
	itemBuf := a.dataFile.GetBytes(itemOffset, sz)
	binary.BigEndian.PutUint32(itemBuf[0:itemHeaderLen], keySize)
	...
	if a.useMapIndex && a.prevUseMapIndex {
		a.mapIndex.Add(hash.Fnv32(a.prevKey), uint32(a.num))
	}
...
func (a *arrayTable) writeItem(key, value []byte) (uint32, error) {
	useMapIndex := a.useMapIndex
	if useMapIndex && !kkv.IsKeyUseMapIndex(key) {
		useMapIndex = false
	}

	switch a.header.version {
	case atVersionPrefixCompress:
		return a.writeItemPrefixCompress(key, value, useMapIndex)
	//case atVersionBlockCompress, atVersionPrefixBlockCompress:
	//	return a.writeItemBlockCompress(key, valueOffset)
	case atVersionBitrieCompress:
		return a.writeItemBitrieCompress(key, value)
	default:
		return a.writeItemDefault(key, value, useMapIndex)
	}
}

func (a *arrayTable) writeItemDefault(key, value []byte, useMapIndex bool) (uint32, error) {
	...
	if a.useMapIndex && useMapIndex {
		a.mapIndex.Add(hash.Fnv32(key), uint32(a.num))
	}

	return sz, nil
}

func (a *arrayTable) writeItemPrefixCompress(key, value []byte, useMapIndex bool) (uint32, error) {
	if a.prevKey == nil && a.prevValue == nil {
		a.prevKey = make([]byte, 0, 128)
		a.prevKey = append(a.prevKey[:0], key...)
		a.prevValue = value
		a.prevHasShared = false
		a.prevUseMapIndex = useMapIndex
		a.sharedKey = make([]byte, 0, 64)
		return 0, nil
	}
...
func (a *arrayTable) writeFinishNonBlockCompress() error {
	...
	intIndexOffset, err := a.dataFile.alloc(intSize)
	if err != nil {
		return err
	}
	intIndexBuf := a.dataFile.GetBytes(intIndexOffset, intSize)
	intIndexPos := 0
	...
	if a.useMapIndex {
		length := a.mapIndex.Length()
		if length > 0 {
			mapSize := a.mapIndex.Size()
			mapIndexOffset, err = a.dataFile.alloc(mapSize)
			if err != nil {
				return err
			}
			a.mapIndex.SetWriter(a.dataFile.GetBytes(mapIndexOffset, mapSize))
...
//func (a *arrayTable) writeItemBlockCompress(key []byte, valueOffset uint32) (uint32, error) {
//	if a.block == nil {
//		a.block = newPageBlock(a.header.version, a.blockSize)
//	}
//
//	a.block.writeItem(key, valueOffset)
//
...
//		offset, err := a.keyFile.Alloc(sz)
//		if err != nil {
//			return 0, err
//		}
//
//		a.itemsOffset = append(a.itemsOffset, offset)
//		itemBuf := a.keyFile.GetBytes(offset, sz)
...
//
//	intSize := a.getIntIndexSize()
//	intIndexOffset, err := a.keyFile.alloc(intSize)
//	if err != nil {
//		return err
//	}
//	intIndexBuf := a.keyFile.GetBytes(intIndexOffset, intSize)
//	intIndexPos := 0
//	for i := range a.itemsOffset {
//		binary.BigEndian.PutUint32(intIndexBuf[intIndexPos:intIndexPos+itemOffsetSize], a.itemsOffset[i])
//		intIndexPos += itemOffsetSize
...
//	a.writeHeader()
//
//	if err = a.keyFile.MmapReadTruncate(int(a.size)); err != nil {
//		return err
//	}
//
//	a.dataIntBuffer()
...
func (a *arrayTable) writeFinishBitrieCompress() error {
	tblAlloc := func(size uint32) uint32 {
		offset, _ := a.dataFile.alloc(size)
		return offset
	}

	tblBytes := func(offset uint32, size uint32) []byte {
		return a.dataFile.GetBytes(offset, size)
	}

	tblSize := func() uint32 {
		return a.dataFile.Size()
	}
...
func (a *arrayTable) getItemKey(i int) ([]byte, uint32) {
	...
	keySize := itemSize - itemOffsetSize
	key := a.dataFile.GetBytes(itemOffset+itemOffsetSize, keySize)
	return key, itemOffset
}
...
func (a *arrayTable) getSharedKey(i int, key []byte, sharedCache *sharedInfo) ([]byte, []byte) {
	...
	default:
		...
		sharedKeySize := binary.BigEndian.Uint32(a.dataFile.GetBytes(itemOffset, itemHeaderLen))
		sharedKey := a.dataFile.GetBytes(itemOffset+itemHeaderLen, sharedKeySize)
...
func (a *arrayTable) getKey(i int) ([]byte, []byte) {
	...
	keySize := binary.BigEndian.Uint32(a.dataFile.GetBytes(itemOffset, itemHeaderLen))
	key := a.dataFile.GetBytes(itemOffset+itemHeaderLen, keySize)
...
func (a *arrayTable) getMaxKey() []byte {
	pos := a.num - 1
	key, itemOffset := a.getItemKey(pos)
	if itemOffset == 0 {
		return nil
	}
...
func (a *arrayTable) getKV(i int) ([]byte, []byte) {
	...
	key := a.dataFile.GetBytes(keyOffset, keySize)
	valueSize := itemSize - keySize - itemHeaderLen
	value := a.dataFile.GetBytes(keyOffset+keySize, valueSize)
...
func (a *arrayTable) get(key []byte, khash uint32) ([]byte, bool, internalKeyKind) {
	if a.header.version == atVersionBitrieCompress {
...
func (a *arrayTable) exist(key []byte, khash uint32) (bool, internalKeyKind) {
	if a.header.version == atVersionBitrieCompress {
		if _, exist := a.bitrieIndex.Get(key); exist {
	...
	//pb := pageBlock{}
	//openPageBlock(&pb, blockBuf)
	//_, exist, kind := pb.get(key)
	//return exist, kind
}

func (a *arrayTable) isNonBlock() bool {
	return a.header.version == atVersionDefault || a.header.version == atVersionPrefixCompress
}

func (a *arrayTable) newIter(opts *iterOptions) InternalKKVIterator {
...
func (a *arrayTable) getFilePath() []string {
	return []string{a.dataPath}
}
...
func (a *arrayTable) getFileType() FileType {
	return fileTypeArrayTable
}
...
func (a *arrayTable) findKeyByIntIndex(key []byte) int {
	return binarySearch(a.num, func(i int) int {
		sharedKey1, sharedKey2 := a.getKey(i)
		return atCompare(sharedKey1, sharedKey2, key)
	})
}

func (a *arrayTable) writeIdxToFile() error            { panic("unimplemented") }
func (a *arrayTable) mmapRLock()                       { panic("unimplemented") }
func (a *arrayTable) mmapRUnLock()                     { panic("unimplemented") }
func (a *arrayTable) kindStatis(internalKeyKind)       { panic("unimplemented") }
func (a *arrayTable) set(internalKey, ...[]byte) error { panic("unimplemented") }
func (a *arrayTable) flushFinish() error               { panic("unimplemented") }
func (a *arrayTable) flushIndexes() error              { panic("unimplemented") }
