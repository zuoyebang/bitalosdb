...
var testPath = filepath.Join(os.TempDir(), "tbl")
...
func testNewArrayTable(path string) (*arrayTable, *atOptions) {
	...
	at, err := newArrayTable(path, opts, atCacheOpts)
	if err != nil {
		panic(err)
	}
	return at, opts
...
func TestArrayTable_Stmap(t *testing.T) {
	...
	hindex.SetWriter(tbl.getData(mapOffset))
	if !hindex.Serialize() {
...
//
//	at, atOpts := testNewArrayTable(testTblPath)
...
//	require.Equal(t, at.dataFile.getFileSize(), int64(at.inuseBytes()))
//
...
//			_, exist, _ := a.get(skey, hash.Fnv32(skey))
...
//
//	at1 := testOpenArrayTable(testTblPath)
...
//
//	testInitDir()
//	at, _ := testNewArrayTable(testTblPath)
...
//			usePrefixCompress: false,
//		}
//		at, err := newArrayTable(testTblPath, opts, &atCacheOpts)
//		require.NoError(t, err)
//
//		bt1 := time.Now()
//		for i := 0; i < count; i++ {
...
//
//		fmt.Printf("useMapIndex=%#v; at.size=%dKB; at.tbl.getFileSize=%dKB\n", useMapIndex, at.dataSize/1024, at.dataFile.getFileSize()/1024)
//
//		seekFunc := func(a *arrayTable) {
//			iter := a.newIter(nil)
//			i := 0
...
//		require.NoError(t, at.close())
//
//		at1, err1 := openArrayTable(testTblPath, &atCacheOpts)
...
//
//		fmt.Printf("opts(usePrefixCompress, useBlockCompress)=%#v; at.size=%dKB; tbl.size=%dKB, at.tbl.getFileSize=%dKB; totalNum=%d\n",
//			opts, at.keyFileSize/1024, at.keyFile.Size()/1024, at.keyFile.getFileSize()/1024, at.num)
//
//		seekFunc := func(a *arrayTable) {
//			bt10_0 := time.Now()
//			iter := a.newIter(nil)
...
//				val, exist, _, closer := a.get(skey, hash.Fnv32(skey))
//				require.Equal(t, true, exist)
//				require.Equal(t, kvList[i].Value, val)
//				if closer != nil {
//					closer()
...
//
//			seekNotExist := func(skey []byte) {
//				_, exist, _, closer := a.get(skey, hash.Fnv32(skey))
//				require.Equal(t, false, exist)
//				if closer != nil {
//					closer()
//				}
...
//			blockSize:         32 << 10,
//		}
//		at, err := newArrayTable(testTblPath, option, atCacheOpts)
//		require.NoError(t, err)
//
//		bt1 := time.Now()
//		for i := 0; i < count; i++ {
...
//		}
//
//		fmt.Printf("opts(usePrefixCompress, useBlockCompress, useBitrieCompress)=%#v; at.size=%dKB; tbl.size=%dKB, at.tbl.getFileSize=%dKB; totalNum=%d\n",
//			opts, at.dataSize/1024, at.dataFile.Size()/1024, at.dataFile.getFileSize()/1024, at.num)
//
//		seekFunc := func(a *arrayTable) {
//			i := 0
//			getFunc := func(i int) {
