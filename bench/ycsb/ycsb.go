...
import (
	...
	"main/internal/ackseq"
	"main/internal/randvar"
	"main/internal/rate"
...
type ycsb struct {
	db           DB
	writeOpts    *bitalosdb.WriteOptions
	weights      ycsbWeights
...
func newYcsb(
	...
	y.writeOpts = bitalosdb.Sync
	if disableWAL {
		y.writeOpts = bitalosdb.NoSync
	}
	...
	ops := map[string]int{
		...
		"scan":            ycsbScan,
		"update":          ycsbUpdate,
		"readmodifywrite": ycsbReadModifyWrite,
	}
...
func (y *ycsb) run(db DB, wg *sync.WaitGroup) {
	...
	if latency[ycsbUpdate] == nil {
		latency[ycsbUpdate] = y.reg.Register("Update")
	}
	errStats := map[int]int{
		ycsbInsert:          0,
		ycsbRead:            0,
		ycsbScan:            0,
		ycsbReverseScan:     0,
		ycsbReadModifyWrite: 0,
	}
	...
	for {
		...
		case ycsbReadModifyWrite:
			y.readModifyWrite(db, buf, latency)
		default:
			panic("not reached")
		}
	...
	fmt.Println("numOps:", y.numOps.Load())
	fmt.Println("error stats:", errStats)
}
...
func (y *ycsb) read(db DB, buf *ycsbBuf) error {
...
func (y *ycsb) update(db DB, buf *ycsbBuf) error {
	key := y.nextReadKey(buf)
	...
	for i := 0; i < count; i++ {
		...
		_ = b.Set(key, v, nil)
		if updateNum == 0 {
			fmt.Println("set value sieze", len(v))
		}
		updateNum++
	}
	defer b.Close()
...
func (y *ycsb) updateByKey(db DB, key []byte, buf *ycsbBuf, objs [ycsbNumOps]*namedHistogram) {
	// start := time.Now()
	count := int(y.batchDist.Uint64(buf.rng))
	b := db.NewBatch()
	for i := 0; i < count; i++ {
		_ = b.Set(key, y.randBytes(buf), nil)
	}
	if err := b.Commit(y.writeOpts); err != nil {
		log.Fatal(err)
	}
	_ = b.Close()
	// objs[ycsbUpdate].Record(time.Since(start)) // add to update
}

// 1 我要实现read的延迟都加起来
func (y *ycsb) readModifyWrite(db DB, buf *ycsbBuf, objs [ycsbNumOps]*namedHistogram) {
	// start := time.Now()
	key := y.nextReadKey(buf)
	_, closer, err := db.Get(key)
	if err != nil && err != bitalosdb.ErrNotFound {
		log.Fatal(err)
	}
	if closer != nil {
		closer()
	}
	// objs[ycsbRead].Record(time.Since(start)) // add to read

	y.updateByKey(db, key, buf, objs)
	// objs[ycsbReadModifyWrite].Record(time.Since(start)) // add to readModifyWrite
}
...
func (y *ycsb) tick(elapsed time.Duration, i int) {
	...
	y.reg.Tick(func(tick histogramTick) {
		...
		fmt.Printf("%10s %8s %14.1f %14.1f %8.3f %8.3f %8.3f %8.3f\n",
			tick.Name,
...
func (y *ycsb) done(elapsed time.Duration) {
	fmt.Println("numOps :", y.numOps.Load(), " config:", ycsbConfig.numOps)
	...
	y.reg.Tick(func(tick histogramTick) {
		...
		fmt.Printf("%10s %7.1fs %14d %14.1f %8.3f %8.3f %8.3f %8.3f %8.3f\n",
	...
	fmt.Println("flushed", m.BytesFlushed, "compacted:", m.BytesCompacted)

	fmt.Printf("Benchmarkycsb/%s/values=%s %d  %0.1f ops/sec  %d write  %0.2f w-amp\n\n",
		ycsbConfig.workload, ycsbConfig.values,
		resultHist.TotalCount(),
		float64(resultHist.TotalCount())/elapsed.Seconds(),
		m.BytesFlushed+m.BytesCompacted,
		m.WriteAmp(),
	)
}
