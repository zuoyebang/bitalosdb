...
var ErrNotIndexed = errors.New("bitalosdb: batch not indexed")
...
func (b *Batch) Apply(batch *Batch, _ *WriteOptions) error {
	...
	if len(batch.data) < batchHeaderLen {
		return base.CorruptionErrorf("bitalosdb: invalid batch")
	}
...
func (b *Batch) Get(key []byte) ([]byte, func(), error) {
	if b.index == nil {
	...
	if ik != nil && ik.Kind() == base.InternalKeyKindSet {
		return v, func() { iter.Close() }, nil
	}
...
func (b *Batch) NewIter(o *IterOptions) *Iterator {
	...
	return b.db.newIterInternal(b, o)
}
