...
func BenchmarkWriteSyncVsDirect(b *testing.B) {
	const targetSize = 256 << 20

	var wsizes []int
	if testing.Verbose() {
		wsizes = []int{512, 1 << 10, 4 << 10, 8 << 10, 16 << 10, 32 << 10}
	} else {
		wsizes = []int{512}
	}
		...
		for _, wsize := range wsizes {
			b.Run(fmt.Sprintf("wsize=%d", wsize), func(b *testing.B) {
				run(b, wsize, func(filename string) File {
					...
					return NewSyncingFile(t, SyncingFileOptions{BytesPerSync: 512 << 10})
				})
			})
		}
	})
		...
		for _, wsize := range wsizes {
			b.Run(fmt.Sprintf("wsize=%d", wsize), func(b *testing.B) {
				run(b, wsize, func(filename string) File {
					...
					return NewSyncingFile(t, SyncingFileOptions{BytesPerSync: 0})
				})
			})
		}
	})
